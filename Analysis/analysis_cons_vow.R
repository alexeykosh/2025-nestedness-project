...
# inventories by language families, but this time keeping only the consonants or 
# the vowels

# REQUIRED INPUT FILES:
# 1. Phoible dataset (CLDF format):
#    - values.csv   => Phoneme data per language
#    - languages.csv  => Language metadata (family, ID, etc.)
#
# 2. Phoible segment metadata:
#    - parameters.csv => Segment classification (used to filter vowels/consonants)
#
# OUTPUT FILES:
# - Nestedness results (CSV):
#   ~ 8 CSV files containing simulated and real nestedness values
#   ~ 4 CSV files with significance categories
# - 4 Plots (PNG):
#
# NOTE:
# - The analysis is run separately for vowels and consonants.
...
# build phonological inventory binary matrices for each family. 
...
# First we do the analysis with only vowels and then with consonants.

# This section can be run in one go and will compute both Temperature and NODF
# for vowels and then for consonants 
...
# 1) Remove "Koman" and "Maningrida" for temp metric (full matrices)
# The temp metric doesn't work on full matrices
...
# 4) Save datasets results for temp
...
## ==== For NODF ====
# 1) Process simulated and observed NODF data
...
# 4) SAve final dataset results for NODF
write.csv(obs_nodf_vow, "obs_nodf_vow_1000.csv", row.names = FALSE)

## ==== For Temperature ====
# 1) Process simulated and observed Temperature data
...
# 4) Save final dataset results for temp 
...
## ==== Display and save combined plots for NODF and Temperature ====
...
# 3) Save datasets results for NODF
...
# 3) Save datasets results for temp 
...
obs_nodf_cons <- process_real(df_nodf_cons_r00_r, df_nodf_cons_c0_r, 
                              "NODF", ALPHA_)

# 2) Create combined NODF plot
plot_nodf_cons <- plot_combined(obs_nodf_cons, sim_nodf_cons$sim_summary, 
                                "NODF")
...
obs_temp_cons <- process_real(df_temp_cons_r00_r, df_temp_cons_c0_r, 
                              "Temperature", ALPHA_)

# 2) Create combined Temperature plot
plot_temp_cons <- plot_combined(obs_temp_cons, sim_temp_cons$sim_summary, 
                                "Temperature")
