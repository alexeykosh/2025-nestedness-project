...
#    - values.csv
#      => Contains phoneme occurrence data for each language
#
# NOTE:languages.csv is NOT required in this version, since no family grouping is used.
#
# OUTPUTS:
# - CSVs with nestedness results:
#     * nodf_results_all_lang_r00.csv
#     * nodf_results_all_lang_c0.csv
#     * temp_results_all_lang_r00.csv
#     * temp_results_all_lang_c0.csv
# - Distribution plots comparing observed vs. simulated results:
#     * dist_nodf_global.png
#     * dist_temp_global.png
...
## ==== Function to select one inventory per language ====
## (same logic as the familiy analysis)
...
nested_test_nodf_full <- function(df_values, shuffling_type = 'r00') {
  ...
  stats <- res$statistic$statistic   # the 3 NODF values (rows, columns and global)
  pvals <- res$oecosimu$pval[3] # Take only the p value of NODF global   
...
plot_distribution_global <- function(df_r00, df_c0,
  ...
  ggplot(df_density, aes(x, y, fill = baseline, color = baseline)) +
    ...
    scale_color_manual(
      name   = "Baseline",
      values = c("r00" = "#56B", "c0" = "orange") # r00 in blue, c0 in orange
    ) +
    labs(
      x     = x_label,
      y     = "Density"
