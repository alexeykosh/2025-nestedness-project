...
#-------------------------------------------------------------------------------
#  II. Matrix Analysis and Visualization
#-------------------------------------------------------------------------------

# I'll certainly put these functions with the others at the top of the code, 
# but for the moment I'm leaving them here to avoid mixing functions that work 
# (those for nestedness) with some that might not work (those for matrices)

# Some of these functions / bits of code are not essential for the final analysis
# and can be deleted (I wanted to see how we can manipulate matrices)
...
#-------------------------------------------------------------------------------
# Panel for small families matrices
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Panel for big families matrices
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Display a single family matrix (unsorted and sorted)
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Panel of the simulated matrices (r00 and c0)
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Matrices of the antinested families
#-------------------------------------------------------------------------------

## Observe the "antinested" families
antinested_families <- c("Nyulnyulan", 
                         "Chukotko-Kamchatkan", 
                         "Gunwinyguan", 
                         "Chocoan")
# For each family, extract, sort, and create the plot of the binary matrix
observed_plots <- lapply(antinested_families, function(fam) {
  bin_mat <- get_family_matrix(fam)
  sorted_mat <- sort_matrix(bin_mat, iterations = 1000)
  plot_matrix(sorted_mat, fam)
})
# Create a panel
panel_antinested <- arrangeGrob(grobs = observed_plots, 
                                ncol = 2)
grid.arrange(panel_antinested)
# To save the panel:
# ggsave(filename = "panel_antinested_families.png",
#        plot = panel_antinested,
#        width = 12, height = 8, dpi = 300)


#-------------------------------------------------------------------------------
# Look at the details of the nestedness results (nodf + temp) for one family
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Temp results in a dataset to detect hot phonemes for a family
#-------------------------------------------------------------------------------

## Temperature Results for Family "Algic"
# Extract the binary matrix for "Algic" 
algic_matrix <- get_family_matrix("Algic")
